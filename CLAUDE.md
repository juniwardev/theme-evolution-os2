...
🚨 IF a Shopify Dev MCP is registered, call `learn_shopify_api` ONCE when starting work on Liquid themes.

## Run locally

- **Install:** Shopify CLI handles theme dependencies automatically. For any
  Node tooling in this project, run `npm install`.
- **Dev server:** `shopify theme dev` (preview URL printed on startup;
  default base URL is `http://127.0.0.1:9292`).
- **Lint / theme check:** `shopify theme check`
...
handoffs between agents live under:

- **Plans:** `docs/plans/<slug>.md` (written by Architect)
- **Reviews:** `docs/reviews/<slug>-review.md` (written by Plan-Reviewer; ends with verdict `APPROVE`, `APPROVE WITH CHANGES`, or `REJECT`)
- **Implementation notes:** `docs/plans/<slug>-impl-notes.md` (written by Coder)
- **QA reports:** `docs/qa/<slug>-report.md` (written by QA; ends with verdict `PASS`, `FAIL`, or `PASS WITH NITS`)
- **Approval gate:** `docs/qa/<slug>.approved` (zero-byte sentinel file YOU create manually before allowing DevOps to deploy)
- **Coder questions:** `docs/plans/<slug>-coder-questions.md` (if Coder hits ambiguity in the plan)
- **Deploy notes:** `docs/qa/<slug>-deploy.md` (written by DevOps)
- **Postmortems:** `docs/qa/<slug>-postmortem.md` (written by DevOps on deploy failure)

## Per-agent guidance for this project

### Coder
- Run `shopify theme check` after each Liquid/JS/CSS edit. Treat warnings as worth addressing.
- Section and block schema JSON must round-trip cleanly through the theme editor — preserve `presets`, `blocks`, and `settings` types and order when editing existing schemas.
- Commits should be small and reviewable. One section/snippet per commit when possible.
...
- **Smoke test:** open the unpublished theme's preview URL (printed by the CLI) and confirm the storefront loads, the feature renders, and no console errors appear.
- **Publish:** via the Shopify admin (Online Store → Themes → Actions → Publish) AFTER manual verification of the unpublished theme. Do NOT auto-publish from CLI.
- **Rollback:** Shopify retains the previous live theme as "unpublished" automatically. Re-publish that one to revert.

## What NOT to change

These files are merchant-controlled or theme-editor-controlled. Agents must NOT edit them unless the plan explicitly calls for it:

- **`config/settings_data.json`** — merchant's saved theme settings. Editing this overwrites their data.
- **`templates/*.json`** — section order is set by merchants in the theme editor. Editing the order or removing sections will clobber their customizations. Only modify if the plan explicitly requires a template structure change.
- **`locales/*.json`** except `locales/en.default.json` — non-default locale files are typically managed by translators or via the Shopify admin.
- **`.shopify/` directory** — Shopify CLI state, version-controlled but not human-edited.

## Project-Specific Architectural Directives

In addition to the Shopify theme reference below, the following project-specific rules ALWAYS apply. Treat these as non-negotiable directives that override the more permissive language in the reference sections that follow.

### Performance & Core Web Vitals

- **LCP (hero images):** Always render above-the-fold hero imagery with explicit preload:
  ```liquid
  {{ image | image_url: width: 1500 | image_tag: preload: true }}
  ```
- **CLS prevention:** Every `<img>` and `<iframe>` must have explicit `width` and `height` attributes to reserve layout space during page load.
- **Asset location (hard rule):** Keep ONLY `critical.css` and essential always-on static files in the `assets/` directory. All component-specific CSS and JavaScript MUST live inside the relevant `sections/`, `blocks/`, or `snippets/` Liquid files using `{% stylesheet %}` / `{% javascript %}` tags (or `<style>` / `<script>` where the rules in the CSS and JS directive sections below require).

### Liquid Logic & Efficiency

- **No `{% include %}`. Ever.** Always use `{% render %}` for snippets. The `include` tag is deprecated and forbidden in this project.
- **Loop efficiency:** Avoid `all_products` or excessive `metafield` lookups inside `{% for %}` loops. Limit nested iterations. If you find yourself iterating over a large collection inside another iteration, stop and rethink — paginate, denormalize via metafields fetched outside the loop, or move the work server-side.

### Block-first Architecture

- **Block-first design:** Prefer building features as reusable, nestable items in `blocks/` over building larger monolithic sections. Sections should be containers; blocks should be the content units merchants compose.

### SEO & Schema

- **Semantic HTML:** Exactly one `<h1>` per page. Maintain a logical, sequential heading hierarchy (no skipping levels).
- **Structured data:** Use JSON-LD in the `<head>` for rich results. Do not emit duplicate schema triggers — they cause Google Search Console conflicts and conflicting rich result eligibility.
